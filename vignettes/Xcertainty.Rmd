...
## Build sampler
Now we will build a sampler. Always start first with using non-informative priors, which should be appropriate for most datasets. In some cases, using informative priors may be more appropriate, especially for datasets that have high errors and when the model is overparameterized -- see the vignette *Xcertainty_informative_priors* for an example. We'll set the altitudes (`image_altitude`) and object length measurements (`object_lengths`) to cover an overly wide range for our target species.  
```{r}
...
We hope this vignette has been helpful for getting started with organizing your input data and how to view and interpret results. Check out other vignettes to view examples of other samplers, as well as using informative priors. 
