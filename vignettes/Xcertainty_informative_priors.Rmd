...
For this vignette, we'll focus on how to use informative priors using the `independent_length_sampler()`. In most cases, it'll be best to use non-informative priors to build the sampler. In some cases, assigning informative priors can be helpful, especially with lower cost off-the-shelf drones that are more susceptible to high errors. 

We will first show an example using non-informative priors and how to identify faulty posterior outputs. We'll then show some steps for trouble shooting and when informative priors may be appropriate to use. We'll then build a sampler using informative priors and view the outputs. 
