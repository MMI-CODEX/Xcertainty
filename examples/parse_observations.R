...
# reformat data so it can be read by the photogrammetric model
calibration_pkg = flatten_data(
...
# build nimble model: calibration data only
calibration_model = build_model(
  pkg = calibration_pkg, useBarometer = TRUE, useLaser = TRUE
)

# build nimble model to jointly analyze calibration data with whale data
standard_model = build_model(
  pkg = project_pkg, useBarometer = TRUE, useLaser = TRUE
)

# build mcmc samplers for models
standard_sampler = build_mcmc(model = standard_model, pkg = project_pkg)
calibration_sampler = build_mcmc(
  model = calibration_model, pkg = calibration_pkg
)

# evaluate calibration parameters and weights
calibration_grid = build_calibration_grid(
  model = calibration_model, pkg = calibration_pkg, niter = niter, level = 2, 
  verbose = TRUE, samples = as.matrix(calibration_sampler$mvSamples)
)

# build calibrated model
calibrated_obs_model = build_model(
  pkg = project_pkg, useBarometer = TRUE, useLaser = TRUE, 
  calibration_grid = calibration_grid
)

# build sampler for calibrated model
calibrated_obs_sampler = build_mcmc(
  model = calibrated_obs_model, pkg = project_pkg
)
